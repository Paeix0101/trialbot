...
media_groups = {} # (chat_id, media_group_id) → {'messages': list, 'last_time': timestamp}
...
def webhook():
    ...
    if "media_group_id" in msg:
        ...
        if key not in media_groups:
            media_groups[key] = {'messages': [], 'last_time': time.time()}
        media_groups[key]['messages'].append(msg)
    ...
    if "reply_to_message" in msg and text.startswith("/repeat"):
        ...
        if "media_group_id" in replied:
            ...
            while waited < max_wait:
                if key in media_groups and len(media_groups[key]['messages']) > 1:
                    break
                time.sleep(step)
                waited += step
                step = min(step + 0.15, 0.8)
            if key in media_groups:
                album_msgs = sorted(media_groups[key]['messages'], key=lambda m: m['message_id'])
            else:
                album_msgs = [replied]
            print(f"[ALBUM DETECT] chat={chat_id} | mgid={mgid} | items={len(album_msgs)}")
            if len(album_msgs) > 1:
                is_album = True
                result_text = f"**✓ Album detected** ({len(album_msgs)} items)\nWill repeat every {display}."
            else:
                result_text = (
        ...
        threading.Thread(
            target=repeater,
            args=(chat_id, album_msgs, interval, job_ref, is_album),
            daemon=True
        ).start()
...
def index():
    return "Bot is alive!"
# -------------------- Repeater --------------------
def repeater(chat_id, album_msgs, interval, job_ref, is_album=False):
    last_sent_ids = []
    while job_ref["running"]:
        ...
        keyboard = None
        if owner_id:
            keyboard = {"inline_keyboard": [[{"text": "Group Owner", "url": f"tg://user?id={owner_id}"}]]}
        # Determine if text or media
        is_text = len(album_msgs) == 1 and 'text' in album_msgs[0]
        if is_text:
            text_content = album_msgs[0]['text']
            entities = album_msgs[0].get('entities')
            ...
            for msg in album_msgs:
                input_media = None
                if 'photo' in msg:
                    input_media = {'type': 'photo', 'media': msg['photo'][-1]['file_id']}
                elif 'video' in msg:
                    input_media = {'type': 'video', 'media': msg['video']['file_id']}
                if input_media:
                    if 'caption' in msg:
                        input_media['caption'] = msg['caption']
                    if 'caption_entities' in msg:
                        input_media['caption_entities'] = msg['caption_entities']
                        input_media['parse_mode'] = "HTML"
                    media.append(input_media)
            if media:
                if len(media) > 1 or is_album:
                    ...
                    if resp.status_code == 200 and resp.json().get("ok"):
                        ...
                        if keyboard and last_sent_ids:
                            last_mid = last_sent_ids[-1]
                    ...
                    input_media = media[0]
                    method = 'sendPhoto' if input_media['type'] == 'photo' else 'sendVideo'
                    payload = {
                        "chat_id": chat_id,
                        input_media['type']: input_media['media']
                    }
                    if 'caption' in input_media:
                        payload['caption'] = input_media['caption']
                    if 'parse_mode' in input_media:
                        payload['parse_mode'] = input_media['parse_mode']
                    if 'caption_entities' in input_media:
                        payload['caption_entities'] = input_media['caption_entities']
                    if keyboard:
                        payload["reply_markup"] = keyboard
                    resp = requests.post(f"{BOT_API}/{method}", json=payload)
                    if resp.status_code == 200 and resp.json().get("ok"):
                        last_sent_ids = [resp.json()["result"]["message_id"]]
        time.sleep(interval)
# -------------------- Keep Alive --------------------
