...
join_windows = {}           # chat_id → {'last_time': timestamp, 'count': int} (kept but no longer used for welcome)
...
def webhook():
    ...
    if "message" in update:
        ...
        user_id = msg["from"]["id"]
        if not str(chat_id).startswith("-") and text.startswith("/start"):
            match = re.match(r"^/start\s+verify_(-?\d+)$", text)
            if match:
    ...
    # 1. Handle join request (still collect user, but no welcome message in private)
    if "chat_join_request" in update:
        ...
        if len(collected_users) >= 200:
            flush_user_batch()
        # No welcome message / verification prompt sent here anymore
        return "OK"
    ...
    # Album / media group collection (still needed for repeat feature)
    ...
    if "reply_to_message" in msg and text.startswith("/repeat"):
        ...
        if "media_group_id" in replied:
            ...
            album_ids = [replied["message_id"]]
            media_keys = ['photo', 'video', 'animation', 'audio', 'voice', 'document', 'video_note', 'sticker']
            if any(key in replied for key in media_keys):
                is_media = True
...
def repeater(chat_id, message_ids, interval, job_ref, is_album=False):
    ...
    while job_ref["running"]:
        ...
        if last_prompt_id:
            delete_message(chat_id, last_prompt_id)
        ...
        if job_ref["is_media"]:
            # ──────────────── CHANGED PART ────────────────
            # Was: forwardMessages  →  Now: copyMessages
